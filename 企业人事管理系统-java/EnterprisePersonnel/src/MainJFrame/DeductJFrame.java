...
public class DeductJFrame extends JFrame implements ActionListener
{
	JPanel panel;
	JPanel panel2;
	...
	DatabaseConnect dc;
	public DeductJFrame(DatabaseConnect dc)
	{
		super("发放奖金页面");
		...
		panel.add(new JLabel("罚款信息",JLabel.CENTER));
		...
		panel.add(this.button_confirm=new JButton("确认罚款"));
	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource()==button_confirm)
		{
			...
			else
			{
				...
				try {
					...
					JLabel label0=new JLabel("请确认罚款信息",JLabel.CENTER);
					...
					JLabel label3=new JLabel("罚款金额："+d,JLabel.CENTER);
		...
		if(e.getSource()==button_confirm2)
		{
			...
			String sql="use EnterprisePersonnel update Attendance "
					+ "set Deduct=Deduct+"+d+" where No='"+str+"' and year="+year+" and month="+month;
			try {
				dc.executeUpdate(sql);
				JOptionPane.showMessageDialog(null, "罚款成功");
				this.dispose();
			} catch (SQLException e1) {JOptionPane.showMessageDialog(null, e1.getMessage());}
		}
